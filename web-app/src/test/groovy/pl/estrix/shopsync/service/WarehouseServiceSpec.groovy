...
class WarehouseServiceSpec extends Specification {
    ...
    def executorMock = Mock(WarehouseCommandExecutor)

    def service = new WarehouseServiceImpl(executorMock)

    def "should use the executor to fetch list of Warehouse by id"() {
        given:
    ...
    def "should use the executor to fetch list of Warehouse by id without order"() {
        given:
            def pagingRequest = preparePagingRequest()
    ...
    def "should use the executor to fetch one Warehouse by id"() {
        given:
    ...
    def "should use the executor to save Warehouse"() {
        given:
            def dto = findDtoMock()
            dto.setId(null)
    ...
    def "should use the executor to update Warehouse"() {
        given:
            def dto = findDtoMock()
            1 * executorMock.getById(_) >> findDtoMock()
    ...
    private WarehouseDto findDtoMock(){
    ...
    private ListResponseDto<WarehouseDto> findListDtoMock() {
        List<WarehouseDto> list = new ArrayList<>()
        list.add(findDtoMock())
        ListResponseDto<WarehouseDto> responseList = new ListResponseDto()
